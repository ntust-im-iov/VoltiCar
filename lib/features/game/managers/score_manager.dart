...
class ScoreManager {
  ...
  final _comboStreamController = StreamController<int>.broadcast();

  /// 分數更新的 Stream
  Stream<int> get scoreStream => _scoreStreamController.stream;

  /// 連擊更新的 Stream
  Stream<int> get comboStream => _comboStreamController.stream;
  ...
  void updateGameTime(double dt) {
    _gameTime += dt;
    // 每秒增加基礎分數
    final newScore = _currentScore + (scorePerSecond * dt).round();
  ...
  void addEventScore(int baseScore, bool isCorrect) {
    ...
    if (isCorrect) {
      _correctAnswers++;
      _comboCount++;
      _comboStreamController.add(_comboCount);
