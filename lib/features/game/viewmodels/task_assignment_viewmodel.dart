...
class TaskAssignmentViewModel extends ChangeNotifier {
    ...
    try {
      ...
      for (final playerTask in activeTasks) {
        ...
        if (existingAcceptedTasks.containsKey(playerTask.taskId)) {
          ...
          try {
            matchingTask = _assignmentTasks
                .firstWhere((task) => task.taskId == playerTask.taskId);
          } catch (e) {
            // 從任務ID中獲取任務類型和名稱
  ...
  Future<void> acceptTask() async {
    ...
    try {
      ...
      final success =
          await _taskAcceptViewModel.acceptTask(_selectedTask!.taskId);
      ...
      if (success) {
        ...
        _updateTaskState(
            isLoading: false, acceptError: errorMessage, isSuccess: false);
      }
  ...
  Future<void> abandonTask() async {
    ...
    if (!canAbandonTask(taskToAbandon)) {
      _updateTaskState(
          isLoading: false, acceptError: '故事模式任務不可放棄', isSuccess: false);
      return;
    }
    ...
    try {
      ...
      // 呼叫放棄任務服務，使用taskId
      await _taskAbandonService.abandonTask(playerTask.taskId);
