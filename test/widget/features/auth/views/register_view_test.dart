...
void main() {
  ...
  testWidgets('輸入有效內容應調用註冊方法', (WidgetTester tester) async {
    ...
    // 找到輸入框並輸入有效內容
    final accountField = find.widgetWithText(TextFormField, '帳號');
    final emailField = find.widgetWithText(TextFormField, '電子郵件');
  ...
  testWidgets('點擊驗證按鈕應調用郵件驗證方法', (WidgetTester tester) async {
    ...
    verify(mockAuthViewModel.sendEmailVerification('test@example.com')).called(1);
